...
///
/// @brief Function to initialise qvoronoi and qhull library.
///        Instructions for Qhull:
///        You need to copy all of the source code files from
///        libqhull folder and then rename the .h->.hpp & .c->cpp
///        in order to compile them as C++ code. In Visual studio
///        you must choose compile as C++ code. This file is is qvoronoi 
///        folder.
///
/// @param[in] argc Number of parameters qvoronoi plus 1 for the name
/// @param[in] argv List of parameters for qvoronoi eg qvoronoi -o -Fv
/// @param[in] input Main input file name for qvoronoi initialisation
/// @param[in] output Main ouput file name for qvoronoi initialisation
/// @return
///
/// @internal
///	Created:		28/12/2013
...
void qvoronoi(int argc, char *argv[], FILE* input, FILE* output) {
  ...
  qh_init_A(input, output, stderr, argc, argv);  /* sets qh qhull_command */
